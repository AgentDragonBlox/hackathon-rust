...
As of the last sync, Person 3's orchestrator runs the full loop —
`offers → clear_market → validate (grid) → settle` — against this
...
`/agents/settle`? If not, `/agents/settle` will record the originally
proposed amount rather than what was actually approved, and the
blockchain ledger will silently be wrong for any partially-approved
action. Worth one explicit test case with a partial approval, not just
the full-approval happy path, before treating this loop as done.

## Open question for Person 1 specifically

*(Likely resolved — Person 3 confirms their orchestrator's loop includes
a `validate` step against Person 1's grid engine. If that's a real
endpoint now, this section can be deleted; leaving the original ask below
for context until someone confirms.)*

Nothing in this service validates electrical feasibility — that's your
job, per `contracts.py`'s `ValidationResult` model. This service doesn't
yet know what endpoint to call to get a `ValidationResult` back for a
`ProposedAction`. **Does your grid engine expose (or plan to expose) a
`POST /grid/validate` (or similar) that Person 3's orchestrator calls
between `clear_market` and `settle`?** If yes, tell Person 3's AI
assistant its exact route and shape so `AgentClient`'s sibling
`GridClient` can be built the same way this doc was built for this
service. If you're using AI to build this, feel free to point it at this
document as an example of the level of detail (real tested payloads, not
assumed ones) to produce for your own handoff doc.
