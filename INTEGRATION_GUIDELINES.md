...
Each service now has its own `INTEGRATION.md`
...
- If you notice a call-order or shape assumption here that looks wrong or
  has changed, flag it in this file explicitly, don't silently reconcile
  it on your own — three separately-built codebases silently
  self-correcting in different directions is exactly the failure mode
...
Both are correct and tested *for their own service*. The risk is
pattern-matching from one to write a client for the other and sending
the wrong shape to one of them. **Not retroactively unifying this** — both are already tested and working; changing either risks
...
canonical sequence end-to-end (not a guess — `grid-infra` and
`agent-engine` were merged locally in a scratch copy, both servers were
started, and `offers` → `clear_market` → `grid/validate` were called for
...
- **RESOLVED — Person 3 confirmed:** the orchestrator already substitutes
  `adjusted_kw_amount` for `kw_amount` before calling `/agents/settle` on
  a partial approval. Built in from the start on their side, verified in
  their code. No longer open.
- **RESOLVED — Person 3's call, as demo owner:** `grid_engine` does
  **not** need an endpoint to commit a settled trade back into its own
  state. The orchestrator already holds the canonical trade/blockchain
  history for dashboard purposes, so `GET /grid/state` not reflecting a
  settled trade is fine for the demo — no new `POST /grid/execute` (or
  similar) needed. `grid_engine/INTEGRATION.md`'s "no endpoint to commit
  an executed action" gap row still describes the *code* accurately, but
  is no longer something the team needs to build.
